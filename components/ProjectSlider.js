...
export default function ProjectSlider({ project_data, number }) {
    ...
    return (
        <div class="carousel carousel-center bg- rounded-box max-w-[1500px] w-[95%] space-x-4 p-4">
            {
                project_data.slice(0, number).map((project, k) => (
                    <div key={k} class="carousel-item max-w-md rounded-box bg-base-100">
                        <ProjectCard project_data={project} img={false} />
                    </div>
                ))
            }
            
            <div class="carousel-item max-w-md rounded-box bg-base-100">
